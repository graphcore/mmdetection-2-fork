...
class GridAssigner(BaseAssigner):
    ...
    def __init__(self,
        ...
        if isinstance(neg_iou_thr, float):
            self.neg_iou_thr = (-0.1, neg_iou_thr)
    ...
    def tmp_aux(self,
        ...
        if extra_input is not None:
            extra_input = (extra_input.mean() * 10).long()
            labels = labels  + extra_input
        assign_result = AssignResult(num_gts, gt_inds, max_overlaps, labels)
        return assign_result

    def statically_assign(self,
                          bboxes,
                          box_responsible_flags,
                          gt_bboxes,
                          gt_labels=None):
    ...
    # @hdDebug.set_func_io_recorder(save_path='assign_test_data.pkl', save_rounds=10)
